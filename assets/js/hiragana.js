document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadUserProgress() {
        const saved = localStorage.getItem('hiraganaMasterProgress');
        if (saved) {
    ...
    function saveUserProgress() {
        ...
        localStorage.setItem('hiraganaMasterProgress', JSON.stringify(userProgress));
    }

    function updateProgress(isCorrect) {
        if (!currentKana) return;

        // Use prefixed index as ID for hiragana
        const idx = kanaData.indexOf(currentKana);
        if (idx === -1) return;
        const id = `hiragana-${idx}`;
    ...
    function startGame() {
        ...
        if (learningMode === 'weak-first') {
            // Count weak points that belong to hiragana
            const hiraganaWeakPoints = userProgress.weak.filter(id => typeof id === 'string' && id.startsWith('hiragana-'));
            quizLength = hiraganaWeakPoints.length;
            
            if (quizLength === 0) {
                showToast("You have no weak points in Hiragana yet! Try other modes first.", 'warning', 'No Weak Points Found');
                return;
            }
        ...
        if (learningMode === 'random-range') {
            ...
            // Map "hiragana-0" back to index 0
            availableIndices = userProgress.weak
                .filter(id => typeof id === 'string' && id.startsWith('hiragana-'))
                .map(id => parseInt(id.replace('hiragana-', ''), 10));
        }
    ...
    function displayNewKana() {
        ...
        if (isRepetitionMode && currentKana) {
            ...
            // questionsAnswered is already incremented in checkAnswer, but wait
            // In hiragana.js, questionsAnswered is incremented in checkAnswer.
        } else {
        ...
        if (idDisplayEl && currentKana) {
            // For hiragana/katakana, we can use the index or a custom ID
            // The JSON doesn't have IDs, so we use the index + 1
            const idx = kanaData.indexOf(currentKana);
